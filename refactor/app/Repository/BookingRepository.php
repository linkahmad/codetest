...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($jobs) {
            ...
            $normalJobs = $normalJobs->each(function ($item) use ($user_id) {
                $item['usercheck'] = Job::checkParticularJob($user_id, $item);
            })->sortBy('due')->all();
        }

        return ['emergencyJobs' => $emergencyJobs->all(), 'normalJobs' => $normalJobs, 'cuser' => $cuser, 'usertype' => $usertype];
    }
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type != env('CUSTOMER_ROLE_ID')) {
            return $this->errorResponse('fail', "Translator can not create booking");
        }

        // Validate 'from_language_id'
        if (!isset($data['from_language_id'])) {
            return $this->errorResponse('fail', "Du måste fylla in alla fält", "from_language_id");
        }

        // Validate immediate bookings
        if ($data['immediate'] == 'no') {
            // Additional validations for non-immediate bookings
            $requiredFields = ['due_date', 'due_time', 'customer_phone_type', 'duration'];
            foreach ($requiredFields as $field) {
                if (!isset($data[$field]) || empty($data[$field])) {
                    return $this->errorResponse('fail', "Du måste fylla in alla fält", $field);
                }
            }
        } else {
            // Set immediate booking values
        ...
        // Handle other conditions and assignments

        return $response;
    }

    // Function to handle error responses
    private function errorResponse($status, $message, $fieldName = null)
    {
    ...
    public function storeJobEmail(array $data)
    {
        ...
        $name = $this->getRecipientName($job->user);
    ...
    /**
     * Get the recipient's name for the email.
     *
     * @param User $user
     * @return string
     */
    ...
    public function jobToData(Job $job): array
    {
        ...
        // Add job_for conditions here based on the 'certified' value
    ...
    public function jobEnd($postData = [])
    {
        ...
        $interval = $completedDate->diffAsCarbonInterval(Carbon::createFromFormat('Y-m-d H:i:s', $dueDate));
        ...
        Event::fire(new SessionEnded($job, ($postData['userid'] == $job->user_id) ? $translatorJob->user_id : $job->user_id));
     ...
     * @param array $data
     */
    private function sendSessionEndedEmail(string $email, string $name, string $subject, string $template, array $data)
    {
     ...
     * @return array
     */
    public function getPotentialJobsForTranslator($userId)
    {
        ...
        $userLanguageIds = UserLanguages::where('user_id', $userId)->pluck('lang_id')->all();
        $gender = $userMeta->gender;
        $translatorLevel = $userMeta->translator_level;

        $jobIds = Job::getJobs($userId, $jobType, 'pending', $userLanguageIds, $gender, $translatorLevel);
        
        $jobIds = $jobIds->filter(function ($jobId) use ($userId) {
            $job = Job::find($jobId);
            $jobUserId = $job->user_id;
            $checkTown = Job::checkTowns($jobUserId, $userId);
            return ($job->customer_phone_type !== 'no' && $job->customer_physical_type === 'yes' && $checkTown);
        });
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        switch (true) {
            case $job->customer_physical_type == 'yes' && $job->customer_phone_type == 'no':
                ...
                break;
            case $job->customer_phone_type == 'yes':
                $message = $phoneJobMessageTemplate; // Phone job or both
                break;
        }
    ...
    private function getTranslatorLevel($certified)
    {
        ...
        if (!empty($certified)) {
            if ($certified == 'yes' || $certified == 'both') {
                ...
                $translatorLevel[] = 'Certified with specialization in law';
                $translatorLevel[] = 'Certified with specialization in health care';
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        // Common filters for both conditions
    ...
    public function bookingExpireNoAccepted()
    {
        $languages = Language::where('active', 1)->orderBy('language')->get();
        $requestData = Request::all();
        $allCustomers = User::where('user_type', 1)->pluck('email')->all();
        $allTranslators = User::where('user_type', 2)->pluck('email')->all();
        ...
        if ($cUser && ($cUser->is('superadmin') || $cUser->is('admin'))) {
            $allJobs->where('ignore_expired', 0)
